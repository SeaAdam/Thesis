...
if ($Password !== $ConfirmPassword) {
    $_SESSION['errorRegistration'] = "Passwords do not match.";
...
if (!preg_match('/^(?=.*\d)(?=.*[a-zA-Z]).{8,}$/', $Password)) {
    $_SESSION['errorRegistration'] = "Password must be at least 8 characters long and contain both numbers and letters.";
    header('Location: index.php');
    exit();
}
...
if ($check_stmt->num_rows > 0) {
    $_SESSION['errorInformationAlreadyRegistered'] = "The information is already registered.";
    $check_stmt->close();
    $mysqli->close();
    header('Location: index.php');
    exit();
...
if ($stmt->execute()) {
    ...
    $_SESSION['cancelRegistered'] = "There was a problem with registration.";
}
